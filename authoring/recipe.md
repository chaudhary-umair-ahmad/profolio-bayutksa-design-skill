...
| What is the shell? | `kb/pages/_shell.html` |
| Which components, with which props? | `kb/design/components/<id>.html` |
| What are the real strings? | `kb/product/copy/<area>.html` |
| What spacing and size steps exist? | `kb/design/scales.html` |
| What colour depends on state? | `kb/design/colours.html` |
| What are the table's columns and rules? | `kb/design/listing-table.html` |
| Which weights actually ship? | `kb/design/fonts.html` |
| Is this value an antd default? | `kb/design/antd-css.html` |
| Is the surface behind a flag? | `kb/product/flags.html` |
...
`scripts/extract-layers.mjs` distils these into `kb/design/`. If a page
...
# save both to data/live/<route>.<side>.probe.json
npm run diff
```
...
data/fixtures/<route>.json       invent the data, keep the shapes
data/live/<route>.capture.json       what the extension writes
```
