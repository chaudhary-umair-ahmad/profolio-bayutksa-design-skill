...
writeFileSync(join(ROOT, 'data/antd-css.json'),
  JSON.stringify({ antdVersion: version, generated: new Date().toISOString(), selectors: json }, null, 2));
writeFileSync(join(ROOT, '.build/references/tokens/antd-css.md'),
...
product wins — see the component register. Everything here is the floor.
...
console.log(`  data/antd-css.json — ${Object.keys(json).length} selectors from antd ${version}`);
console.log(`  antd-css (kb)     — ${Object.keys(WANT).length} component groups`);
