...
writeFileSync(join(ROOT, 'references', 'tokens', 'antd.json'),
  JSON.stringify({ antdVersion: version, appPins, generated: new Date().toISOString(), token: t }, null, 2));

writeFileSync(join(ROOT, 'references', 'tokens', 'antd.md'),
...
Full output: \`references/tokens/antd.json\` (${Object.keys(t).length} tokens).
...
console.log(`  references/tokens/antd.json — ${Object.keys(t).length} tokens from antd ${version}`);
console.log(`  references/tokens/antd.md   — the ${Object.keys(ANSWERS).length} groups the report could not answer`);
